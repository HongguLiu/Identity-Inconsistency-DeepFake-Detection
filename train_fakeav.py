...
from datasets import FakeAVDataset, FakeAVDataset_test

from model.base_model import Arcface_model, LSTM_model, get_model
...
def train_epoch(epoch, num_epochs, data_loader, model_id, model_lstm, criterion, optimizer, log_path=None):
    ...
    model_id.eval() #Arcface_model for inference purposes
    model_lstm.train()
    losses = AverageMeter()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sequence_length', '-sq', type=int, default=100, help='Number of Sequence Lengths')
    parser.add_argument('--bidirectional', type=bool, default=False)
    parser.add_argument('--train_file', type=str, default='/nas/home/hliu/Datasets/FakeAVCeleb_v1.2/data_list/train.txt', help='The file list of training examples')
    parser.add_argument('--val_file', type=str, default='/nas/home/hliu/Datasets/FakeAVCeleb_v1.2/data_list/val.txt', help='The file list of validation examples')
    parser.add_argument('--test_file', type=str, default='/nas/home/hliu/Datasets/FakeAVCeleb_v1.2/data_list/test.txt', help='The file list of test examples')
    ...
    train_dataset = FakeAVDataset(args.train_file, args.sequence_length, train_transforms)
    
    val_dataset = FakeAVDataset(args.val_file, args.sequence_length, test_transforms)

    test_dataset = FakeAVDataset_test(args.test_file, args.sequence_length, test_transforms)
    # import pdb
    # pdb.set_trace()
    ...
    if args.balance_weight:
        class_weights = torch.from_numpy(np.asarray([1,0.1])).type(torch.FloatTensor).cuda()
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print_log(classification_report(v_true, v_pred, labels=[0, 1], target_names=['Real', 'Fake']), log_path)
        ...
        print_log(classification_report(t_true, t_pred, labels=[0, 1], target_names=['Real', 'Fake']), log_path)
